...
- Font loading: font-display: swap + critical fonts <link rel="preload">
...
:root {
  ...
  --bg-warm:   #ECE7DA;   /* warm card */
  --bg-card:   #EAE6DC;   /* card/spec block */
  --ink:       #1F1E1B;   /* primary text */
  --ink-soft:  #5A5750;   /* secondary text */
  ...
  --a-tech:    #5B7A9A;   /* Tech Wing · steel blue-gray */
  --a-think:   #C15F3C;   /* Think Wing · red earth (same as main accent) */
  --a-create:  #7A8C5F;   /* Create Wing · moss green */
}
```
...
Each route is an independent React page component, configured via `<Routes>`. DO NOT use modal overlays to simulate new pages — article detail, about, and wings must all be independent routes where the URL bar changes, content is shareable, and back-button works.
...
Vol. 01 · One-author archive               est. 2026
```
...
1. Container progress `p ∈ [0,1]`, ease-in-out: `eased = p<.5 ? 2p² : 1 - (-2p+2)²/2`
2. Door frame height from `38vh` → `110vh` (slightly over, edges push off-screen)
3. Image `transform: scale(1.04 → 1.18)`
4. Image `filter: brightness(.95 → .40) saturate(.88 → .68)`
5. Warm cream tint overlay opacity `1 → 0`
6. When eased > 0.55, nav switches to on-dark
...
[plate-rule] B.Museum · Vol. 01 · One-author archive  ／  est. 2026
```
...
Bottom separator line: `© 2026 B.Museum · Built solo  ／  Vol. 01 · Est. 2026 · Walk through the mind.`
...
guestbook:index:published    → array<id>，按时间倒序
guestbook:index:pending      → array<id>，等待 24h 自动发布
...
4. **字体加载优化**
   - 当前全部用 `@import` 加载（fonts.css），改为 `<link rel="preload" as="font" crossorigin>` 关键字体（Noto Serif SC 200、Inter 500）
...
5. **Threshold 图片**
   ...
   - 通过 `<link rel="preload" as="image">` 预加载主图

6. **SEO 资产（纯前端改动，不依赖 Edge Functions）**
   - 各页面组件设置 `document.title`（或引入 `react-helmet-async`）
   - `index.html` 中的 `<meta>` 作为默认 fallback
   - 各路由独立 `<meta name="description">` 和 `<meta property="og:*">`
...
3. **修代码层小 bug**（与 prompt 一致性相关）：
   ...
   - 校正 About §05/§06/§07 文本与 prompt 对齐
   - Footer 右列补 RSS / Atom 链接
4. **Edge Function：Visitor Ticket** — 创建 `functions/api/visitor/issue.ts`，KV 原子递增 `counter:visitors`。前端 `useVisitorTicket` 改为调 `/api/visitor/issue`，移除 `Math.random()`。
5. **Edge Function：Guestbook GET/POST** — 创建 `functions/api/guestbook/index.ts` + `blocklist.json`。前端 `Guestbook.tsx` 改走 fetch，移除 SEED 与 30s autoPublish。
6. **Cron：24h 自动发布** — Edge Function 定时任务每小时扫描 `guestbook:index:pending`，超过 24h 改为 `published`。
7. **清理冗余**：删除 `components/hero/`、`components/threshold/`、`components/hall-preview/`、`components/shared/` 四个空目录。
8. **SEO 资产**：各页面 `document.title` + meta description + og 标签；生成 `sitemap.xml`、`feed.xml`。
9. **OG Image Edge Function**：按 wing slug / article slug 动态生成 1200×630 卡片图。
10. **性能优化**：Threshold 图片下载转 WebP + LQIP；字体改为 `<link rel="preload">`；`vite.config.ts` 配 vendor 拆分；验证 main bundle < 80KB gzip、LCP < 1.5s。
11. **部署**：build → 部署到 EdgeOne Pages → 绑定自定义域名 → 配置 KV namespace（`bmuseum-guestbook`、`bmuseum-visitors`）→ 验证 History fallback 与 CDN 缓存策略生效。

VERIFICATION CRITERIA (Self-Check Each Step)

按优先级分层。**P0 不通过不能部署**。

**P0 — 必须通过（部署门槛）**
- [ ] 8 个 URL 全部独立访问、刷新不丢状态、URL 栏正确变化
- [ ] SPA fallback 工作：任意路径刷新返回 `index.html`
- [ ] Guestbook 提交走 Edge Function + KV（不再依赖 localStorage / SEED）
- [ ] Visitor `ticket_no` 来自 KV `counter:visitors` 原子递增（不再是 `Math.random()`）
- [ ] 主背景始终 `#F0EEE6`；Hall Preview / Footer / Room Marker 是唯一的 `#1F1E1B` 暗区；Guestbook 是唯一的 `#F4ECD6` 牛皮纸区；无第四色温区
- [ ] 无 particle backgrounds / WebGL / dark mode toggle / i18n / 模态框假装路由 / fake guestbook 数据
- [ ] Article Detail 是真路由（`/article/:slug`），不是 modal
- [ ] Wing accents 仅作用于 hover / tag / active 指示，不影响主背景

**P1 — 应该通过（部署前尽量完成）**
- [ ] LCP < 1.5s（Lighthouse）
- [ ] Main bundle < 80KB gzip
- [ ] 所有字体 `font-display: swap`，关键字体（Noto Serif SC 200 / Inter 500）`<link rel="preload">`
- [ ] Threshold 图片转 WebP + LQIP placeholder
- [ ] `sitemap.xml` 可访问，列出全部 8 路由 + 18 篇文章
- [ ] `feed.xml` 可访问（RSS 2.0）
- [ ] 各页面独立 `<title>` 与 `<meta name="description">`
- [ ] OG Image 按 wing/article 动态生成
- [ ] Hero 三行 h1 错序入场；Threshold 滚动门开效果完整；Nav 在 Threshold 进入暗区时切 on-dark；首次访问 2.5s 后票根弹出
- [ ] 所有路由切换是 200ms cross-fade，**不**重播 Hero 入场动画
- [ ] 所有中文楷体强调（如"收藏""想法""懂你""真实"）使用 LXGW WenKai

**P2 — 可选（后续迭代）**
- [ ] Guestbook 24h 自动发布（Cron Edge Function）
- [ ] 客户端路由 transition 动画微调
- [ ] EdgeOne Web Analytics 接入
- [ ] Threshold 图片 srcset 多分辨率
- [ ] `articles.json` / `wings.json` 后续迁移到 `/api/articles`、`/api/wings/:slug`

CLEANUP TASKS

构建完成、Edge Function 上线后必须执行的清理：

1. **删除空组件目录**（功能已内联在 GrandHall.tsx）：
   - `src/components/hero/`
   - `src/components/threshold/`
   - `src/components/hall-preview/`
   - `src/components/shared/`
2. **Guestbook.tsx**：移除 `SEED` 常量、移除 30s autoPublish、移除 localStorage fallback 分支（保留空数组兜底即可）
3. **useVisitorTicket.ts**：移除 `Math.random()` 票号生成；保留 localStorage 仅作为客户端缓存（不再作为权威源）
4. **About.tsx**：抽出硬编码数据到 `src/data/about.json`
5. **Foyer 段落**（GrandHall 中"出口小语"自定义段落）：保留即可，已是有效设计；若决定移除请同步更新本 prompt

STYLE KEYWORDS (Final Atmosphere Words)

> Warm cream-white · Editorial restraint · Anthropic layout sensibility · Noto Serif 200 large-text whitespace · Kaiti as Chinese italic · Fraunces English italic subtitles · One cinematic door-opening entrance · Grain paper texture · Red earth accent unified · Three wings subtly differentiated but never喧宾夺主 · Ticket narrative · Museum is space not flow

---

Final word: This museum has one curator, opens one volume per year. It doesn't need to chase trends — it only needs to make someone feel, in the first second, that "someone was serious here."

— B.Museum · Vol. 01 · Curator's Master Brief
